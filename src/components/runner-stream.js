...
class RunnerStream extends Component {
  ...
  componentDidMount() {
    ...
    if (!isFeatured) {
      this.timeline.set(this.container.current, { yPercent: 80, opacity: 0 }).play();
    }
  }
  ...
  prepareAnimateIn() {
    this.timeline.set(this.container.current, { yPercent: 105, opacity: 0 }).play();
  }
    ...
    return (
      <div class={style.container}>
        {includeFeaturedIndicator && (
          <FeaturedIndicator className={style.featuredIndicator} runId={runId} />
        )}
        {!isFeatured && (
