...
class RunUpdate extends Component {
  ...
  render() {
    const { update, run, team, game, runner, ready, className } = this.props;

    return (
      <StretchReveal pathId="updateClipPath" rectId="updateClipRect" objectId="updateObject">
        {ready && (
          <div
            class={classNames(style.container, className)}
            style={{ "--color": `#${team.color}` }}>
            <p class={style.text}>
              <strong>{runner.username}</strong> just finished <strong>{game.name}</strong> in{" "}
              <strong>{runTime(run.actual_seconds || 0)}</strong> for <strong>{team.name}</strong>!
            </p>
          </div>
        )}
      </StretchReveal>
...
export default connect(mapStateToProps)(RunUpdate);
