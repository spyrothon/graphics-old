  ...
  _animateIn() {
    this.timeline
      ...
      .fromTo(
        ...
        { opacity: 1, yPercent: 0, ease: "Power4.easeOut" },
        1,
      )
      // Children
    ...
    return (
      <div class={style.content} ref={this.container} style={{ "--color": `#${team.color}` }}>
        ...
        <div class={style.runs}>
          <StaggerIn childClassName={style.staggerElement} timeline={this.childTimeline}>
            {_.map(runIds, (runId) => (
              <Run className={style.run} key={runId} runId={runId} midRow="game" />
            ))}
          </StaggerIn>
        </div>
      </div>
