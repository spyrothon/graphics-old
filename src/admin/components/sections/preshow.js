...
class PreshowSection extends Component {
  ...
  render() {
    const { teams, className } = this.props;
    const { selectedTeamId } = this.state;
    ...
    return (
      <Section className={className} title="Preshow">
        ...
        <Button onClick={() => this.setActiveOverlay()}>Set Active Overlay</Button>

        <Button onClick={() => this.clearSelectedTeam()}>Clear Active Overlay</Button>
      </Section>
    );
  }
}
