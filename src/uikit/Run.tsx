...
import { Account } from "../modules/accounts/AccountTypes";
import { Game } from "../modules/accounts/GameTypes";
import { Run } from "../modules/accounts/RunTypes";
import { Team } from "../modules/accounts/TeamTypes";
...
class Run extends React.Component<RunProps> {
  renderMidRow() {
    const { midRow, game, team } = this.props;

    switch (midRow) {
      case "team":
        return <div class={style.teamName}>{team.name}</div>;
      case "game":
      default:
        return <div class={style.gameName}>{game.name}</div>;
    }
  }

  renderTimeRow() {
    const { run, timeRow } = this.props;
