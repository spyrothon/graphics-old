...
> {
	...
	async connect(
		...
		try {
			const connectionClosedPromise = this.internalEventPromise<
				EventTypes["ConnectionClosed"]
			>("ConnectionClosed");
			const connectionErrorPromise = this.internalEventPromise<
	 ...
	 */
	async callBatch(
		requests: RequestBatchRequest<keyof OBSRequestTypes>[],
		options: {
			haltOnFailure?: boolean;
			excecutionType?: RequestBatchExecutionType;
		}
		...
		await this.message(WebSocketOpCode.RequestBatch, {
			...
			executionType: options.excecutionType,
		});
	...
	protected async createConnection(url: string) {
		const connectionOpenedPromise = this.internalEventPromise(
			"ConnectionOpened"
		);
