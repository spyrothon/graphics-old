...
function isObsRequestField(
	val: OBSRequestField | OBSResponseField
): val is OBSRequestField {
...
const { body: release } = await got(
	"https://api.github.com/repos/obsproject/obs-websocket/releases/latest",
	{
		headers,
		responseType: "json",
	}
...
const { body: protocol } = await got<GeneratedProtocol>(
	`https://raw.githubusercontent.com/obsproject/obs-websocket/${commit}/docs/generated/protocol.json`,
	{
		headers,
		responseType: "json",
	}
...
function generateObsEventTypes(events: OBSEvent[]): string {
	return events
		.map((ev) => {
			...
			return `${ev.eventType}: ${stringifyTypes(
				unflattenAndResolveTypes(ev.dataFields)
			)};`;
		})
		.join("\n");
}
...
function generateObsRequestTypes(requests: OBSRequest[]): string {
	return requests
		.map((req) => {
			...
			return `${req.requestType}: ${stringifyTypes(
				unflattenAndResolveTypes(req.requestFields)
			)};`;
		})
		.join("\n");
}
...
function generateObsResponseTypes(requests: OBSRequest[]): string {
	return requests
		.map((req) => {
			...
			return `${req.requestType}: ${stringifyTypes(
				unflattenAndResolveTypes(req.responseFields)
			)};`;
		})
		.join("\n");
}
...
function unflattenAndResolveTypes(
	inputItems: Array<OBSRequestField | OBSResponseField>
): ObjectType {
...
function getBaseType(inType: string): AnyType {
	...
	if (inType.startsWith(ARRAY_PREFIX)) {
		const subType = inType.substring(
			ARRAY_PREFIX.length,
			inType.lastIndexOf(">")
		);
