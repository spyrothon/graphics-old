...
import { CropTransform, OBSWebsocketConfig, Transition, TransitionSet } from "@api/APITypes";
...
// TODO: figure out custom messages
...
class OBS {
  ...
  async getInputStates(inputNames: string[]) {
    ...
    for (const response of results) {
      ...
      switch (requestType) {
        case "GetInputVolume":
          data["inputVolumeMul"] = response.responseData?.inputVolumeMul;
          data["inputVolumeDb"] = response.responseData?.inputVolumeDb;
          break;
        case "GetInputMute":
          data["inputMuted"] = response.responseData?.inputMuted;
          break;
        case "GetInputAudioSyncOffset":
          data["inputAudioSyncOffset"] = response.responseData?.inputAudioSyncOffset;
          break;
        case "GetMediaInputStatus":
          data["playbackInProgress"] =
            response.responseData?.mediaState === "OBS_MEDIA_STATE_PLAYING";
          break;
        case "GetSourceActive":
          data["videoActive"] = response.responseData?.videoActive;
          data["videoShowing"] = response.responseData?.videoShowing;
          break;
      }
